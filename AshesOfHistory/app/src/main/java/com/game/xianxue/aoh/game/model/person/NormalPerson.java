...
import com.game.xianxue.aoh.interfaces.IPerson;
...
public class NormalPerson extends BasePerson implements IBuffEffect, IPerson, ISkill {
    ...
    ArrayList<BuffEffectBattle> passiveBuffList = null;       // 被动buff列表（被动技能加持的）
    ...
    public int damageReduce;       // 减伤（计算伤害的时候，最后计算）todo 这个还没有计算
    public int dodge;              // 闪避
    ...
    float damageReduceRate;         // 减伤百分比（计算伤害的时候，在damageReduce之前计算）todo 这个还没有计算
    ...
    public NormalPerson(BasePerson basePerson) {
        // 原始资料
        this.personId = basePerson.getPersonId();                   // id唯一标识这个人物
        this.name = basePerson.getName();                           // 名字
        this.namePinyin = basePerson.getNamePinyin();               // 名字拼音
    ...
    public NormalPerson(BasePerson basePerson, int level) {
        ...
        LogUtils.d(TAG, "NormalPerson(): skillStrings=" + this.skillStrings);
    ...
    protected void initSkill() {
        ...
        BuffEffectBattle buffBattle = null;
        passiveBuffList = new ArrayList<BuffEffectBattle>();
        ...
        for (int i = 0; i < this.skillAllLists.size(); i++) {
            ...
            LogUtils.d(TAG, this.getName() + " Lv." + this.level + skillBean.getSkill().getName() + "的解锁等级为:" + skillBean.getUnLockLevel());
            ...
            if (SKILL_NATURE_ACTIVE == skillBean.getSkill().getSkillNature()) {
                ...
                buffBattle = new BuffEffectBattle(buffBase, skillBean.getCurrentSkillLevel(this.level));
    ...
    public void updateAttribute() {
        ...
        calculateBaseAttributeFromPassiveBuffList();// 先处理基础属性的buff，因为面板属性是由基础属性计算出来的
    ...
    protected void resetAllAttribute() {
        ...
        damageReduce = 0;         // 减伤
        attackNumberUp = 0;       // 攻击人物数量提升，这个属性初始为0
    ...
    protected int getNewAttribute(int Initial, int level) {
        return (int) ((float) Initial + (float)(Initial * 0.5f) * (float) level);
    }
    ...
    public ArrayList<BuffEffectBattle> getPassiveBuffList() {
        return passiveBuffList;
    }

    public void setPassiveBuffList(ArrayList<BuffEffectBattle> passiveBuffList) {
