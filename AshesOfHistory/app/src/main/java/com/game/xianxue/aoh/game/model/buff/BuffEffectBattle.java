...
import com.game.xianxue.aoh.utils.TextUtils;

/**
 * Buff 在战斗时的实体类
 */

public class BuffEffectBattle extends BuffBase implements IBuffEffect {
    private static final String TAG = "BuffEffectBattle";
    ...
    public BuffEffectBattle(BuffBase base) {
        ...
        this.superposition = base.superposition;            // buff类型     (0：只触发一次 1：时间内每回合触发一次)
    ...
    public BuffEffectBattle(BuffBase base, int level) {
        this.buffId = base.buffId;                  // buffid
        this.name = base.name;                      // buff名字
        this.introduce = base.introduce;            // buff说明
        this.buff_nature = base.buff_nature;        // buff的性质   (0:被动buff 1:主动buff)
        this.superposition = base.superposition;            // buff类型    （分为攻击buff，辅助buff，恢复buff）
        this.time = base.time;                      // buff持续时间
        this.range = base.range;                    // buff的影响范围
        this.level_up_range = base.level_up_range;              // 每升一级浮动部分的提升
        this.level_up_time = base.level_up_time;                // 每升一级浮动部分的提升
    ...
    public void setLevel(int level) {
        ...
        if (buff_effect.length == buff_constant.length
            ...
            LogUtils.e(TAG, this.getName() + " buff setLevel():的时候出现错误。");
        }
    }
    ...
    public void setDuration(int duration) {
        ...
        if (this.superposition != BUFF_SUPERPOSITION) return;
        ...
        if (buff_effect.length == buff_constant.length
            ...
            LogUtils.e(TAG, this.getName() + " buff setDuration():的时候出现错误。");
        }

        LogUtils.d(TAG,this.getName()+ "持续时间为："+duration + "当前固定部分为:"+buff_constant[0]+" 浮动部分为:"+buff_fluctuate[0]);
    }

    @Override
    public String toString() {
        return "BuffEffectBattle{" +
                "level=" + level +
