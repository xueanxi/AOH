...
import com.game.xianxue.aoh.App;
...
public class DataBaseHelper extends SQLiteOpenHelper {
     ...
     * @param db
     */
    private void createTableFormation(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE " + FormationColumn.tableName + " ( "
                + FormationColumn.id + " INTEGER PRIMARY KEY AUTOINCREMENT,"
                + FormationColumn.formation_id + " INTEGER,"
                + FormationColumn.name + " TEXT,"
                + FormationColumn.introduce + " TEXT,"
                + FormationColumn.max_person + " INTEGER,"
                + FormationColumn.formation_json + " TEXT)");
        insertFormations(db);
    }
     ...
     * @param db
     */
    private void insertFormations(SQLiteDatabase db) {
        // 进行批处理
        List<FormationBase> lists = null;
        db.beginTransaction();
        try {
            lists = XmlUtils.getAllFormations(mContext);
        ...
        for (FormationBase data : lists) {
            db.execSQL(FormationManager.getInsertString(data));
        }
    ...
    private void createTableBuff(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE " + BuffColumn.tableName + " ( "
                ...
                + BuffColumn.superposition + " INTEGER,"
    ...
    private void insertSkillData(SQLiteDatabase db) {
        ...
        for(int i=0;i<lists.size();i++){
            skill = lists.get(i);
            if(skill == null){
                LogUtils.e(TAG,"insertSkillData() skill == null");
            }else{
    ...
    private void createTablePerson(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + BasePersonColumn.tableName
                ...
                + BasePersonColumn.namePinyin + " TEXT,"
