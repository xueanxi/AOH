...
import com.game.xianxue.aoh.game.model.DamgeModel;
...
import com.game.xianxue.aoh.interfaces.ISkill;
import com.game.xianxue.aoh.Log.BattleLog;
import com.game.xianxue.aoh.Log.BuffLog;
...
import com.game.xianxue.aoh.utils.RandomUtils;

import java.util.ArrayList;
import java.util.List;

import static com.game.xianxue.aoh.Log.BattleLog.TAG_A;
import static com.game.xianxue.aoh.Log.BattleLog.TAG_CR;
import static com.game.xianxue.aoh.Log.BattleLog.TAG_IJ;
import static com.game.xianxue.aoh.Log.BattleLog.TAG_MS;
import static com.game.xianxue.aoh.Log.BattleLog.TAG_NA;
import static com.game.xianxue.aoh.Log.BattleLog.TAG_PG;
import static com.game.xianxue.aoh.Log.BattleLog.TAG_RH;
import static com.game.xianxue.aoh.Log.BattleLog.TAG_SA;
import static com.game.xianxue.aoh.Log.BattleLog.TAG_SE;
import static com.game.xianxue.aoh.Log.BattleLog.TAG_ST;


/**
 * 战斗处理引擎 单例
 */
public class BattleEngine implements ISkill, IBuffEffect, ActiveValueManager2.IActiveValueManagerCallBack {
    private static final String TAG = "BattleEngine";
    private static final Object lock = new Object();
    private static BattleEngine mInstance;
    private ActiveValueManager mActiveValueManager;
    private ActiveValueManager2 mActiveValueManager2;
    ...
    public void startBattle2() {
        BattleLog.log(TAG_PG,"开始战斗！！！");
        mActiveValueManager2 = new ActiveValueManager2(mInstance, mPersonsList);
        mActiveValueManager2.setTimeActiveInterval(mTimeActiveIncrese);

        BattleLog.log(TAG_PG,"mIsBattleing:"+mIsBattleing+" ");
        while (mIsBattleing && (isBattleFinish(t1, t2) == 0)) {
            mActiveValueManager2.increaseActiveValue();
            if(mActiveValueManager2.getActivePersons() == null){
                continue;
            }

            List<BattlePerson> mActivePersions = mActiveValueManager2.getActivePersons();
            for(BattlePerson person:mActivePersions){
                if(isBattleFinish(t1, t2) != 0){
                    return;
                }
        ...
        BattleLog.log(TAG_PG,"检测到战斗已经分出胜负，战斗引擎尝试关闭...");
        stopBattle();
    }
    ...
    public void startBattle() {
        BattleLog.log(TAG_PG,"开始战斗！！！");
        mActiveValueManager = new ActiveValueManager(mInstance, mPersonsList);
        mActiveValueManager.setTimeActiveInterval(mTimeActiveIncrese);

        mLogicThread = new SuspendThread() {
            @Override
            public void runLogic() {

                if(!mActiveValueManager.isStart){
                    mActiveValueManager.start();
                }
                ...
                mActiveValueManager.resume();
            }
        };
        mLogicThread.start(mTimePersonAction);
    }

    private void pauseBattleIfNeed(){
        synchronized (lock){
            while (isPauseBattle){
                try {
                    mActiveValueManager.pause();
                    lock.wait();
    ...
    public void stopBattle() {
        BattleLog.log(TAG_PG,"结束战斗！！！");
        if (mActiveValueManager != null) {
            mActiveValueManager.stop();
            mActiveValueManager = null;
        }
    ...
    private static int isBattleFinish(TeamModel t1, TeamModel t2) {
        ...
        if (!isLeftFail && !isRightFail) {
            BattleLog.log(TAG_PG,"战斗还在继续");
            return 0;
        } else if (!isLeftFail && isRightFail) {
            BattleLog.log(TAG_PG,"战斗结束 我军胜利");
            return 1;
        } else if (isLeftFail && !isRightFail) {
            BattleLog.log(TAG_PG,"战斗结束 敌军胜利");
            return 2;
        } else {
            BattleLog.log(TAG_PG,"战斗结束 两败俱伤");
            return 3;
        }
    }
    ...
    public void doAction(BattlePerson actionPerson) {
        ...
        BattleLog.log(TAG_ST,actionPerson.getName() + "技能列表：" + skillStrings.toString());
    ...
    private boolean handlerReduceBuff(BattlePerson actionPerson) {
        ...
        int reduceHp = 0;
        ArrayList<BuffEffectBattle> buffLists = actionPerson.getActiveBuffList();
        if(buffLists != null && buffLists.size()>0){
            for(BuffEffectBattle buff:buffLists){
                for(int i = 0 ;i<buff.getBuff_effect().length;i++){
                    if(buff.getBuff_effect()[i] == BUFF_REDUCE_HP){
                        ...
                        if(actionPerson.getHP_Current() > reduceHp){
                            ...
                            BattleLog.log(TAG_IJ,actionPerson.getName()+"受到["+buff.getName()+"]影响，损失了"+reduceHp+"点生命");
                        }else{
                            actionPerson.setHP_Current(0);
                            BattleLog.log(TAG_IJ,actionPerson.getName()+"受到["+buff.getName()+"]影响，损失了"+reduceHp+"点生命，死亡了！！！");
    ...
    private void handleBuffBeforeAction(BattlePerson actionPerson) {
        ...
        if (buffPassives != null) {
            for (int i = 0; i < buffPassives.size(); i++) {
                ...
                // 处理可以持续叠加的buff
                if (BUFF_SUPERPOSITION == buff.getSuperposition()){
    ...
    private void restoreHp(BattlePerson actionPerson) {
        ...
        if (HP_Current > actionPerson.getHP_MAX()) {
            ...
            BattleLog.log(TAG_RH,actionPerson.getName() + " 自然恢复了" + HP_Loss + "点生命值，生命值满了，当前生命值为"+actionPerson.getHP_MAX());
        } else {
            actionPerson.setHP_Current(HP_Current);
            if (actionPerson.getHpRestore() > 0) {
                BattleLog.log(TAG_RH,actionPerson.getName() + " 自然恢复了" + actionPerson.getHpRestore() + "点生命值,当前生命值为"+actionPerson.getHP_Current());
            } else {
                BattleLog.log(TAG_RH,actionPerson.getName() + " 自然损失了" + actionPerson.getHpRestore() + "点生命值,当前生命值为"+actionPerson.getHP_Current());
            }
        }
    }
    ...
    public void startAttack(BattlePerson actionPerson, boolean isNormalAttack) {
        BattleLog.log(TAG_PG,actionPerson.getName() + "发动攻击");
        BattleLog.log(TAG_ST,actionPerson.display());
        ...
        if (isNormalAttack) {
            ...
            if (skillCurrent == null) {
                LogUtils.e(TAG, "Error !!! 获取普通攻击技能失败，放弃这次普通攻击！！！");
                return;
            }
        } else {
            // 从技能列表里面随机选择出一个技能(排除掉普通攻击)
            if (skillLists == null || skillLists.size() == 0) {
                LogUtils.e(TAG, "Error !!! " + actionPerson.getName() + "的技能列表为空,停止释放技能 return");
                return;
            } else if (skillLists.size() == 1) {
                LogUtils.e(TAG, "Error !!! " + actionPerson.getName() + "的技能列表只有一个普通攻击，放弃技能释放，转为进行普通攻击");
                BattleLog.log(TAG_NA,actionPerson.getName() + "的技能列表只有一个普通攻击，放弃技能释放，转为进行普通攻击");
                ...
                if (tempLists == null || tempLists.size() == 0) {
                    BattleLog.log(TAG_NA,actionPerson.getName() + "想要释放技能，但是没有冷却完毕的技能，所以准备进行普通攻击。");
                    tempLists.clear();
        ...
        List<BattlePerson> personsBeAttackList = new ArrayList<BattlePerson>();            // 被技能攻击的目标
        TeamModel teamBeAttack = null;            // 被技能影响的阵营
        BattlePerson personBeAttack = null;       // 被技能影响的人
        BattleLog.log(TAG_SA|TAG_NA,actionPerson.getName() + " " + skillCurrent.getName() + "Lv." + skillCurrent.getLevel() + " 的距离为:" + effectRange + ",人数为:" + effectNumber);

        List<BattlePerson> tempList = new ArrayList<>();   // 存放临时数据的列表1
        List<BattlePerson> tempList2 = new ArrayList<>();  // 存放临时数据的列表2
        ...
        if (t1.getCamp() == effectCamp) {
            teamBeAttack = t1;
        } else if (t2.getCamp() == effectCamp) {
            teamBeAttack = t2;
        } else {
            LogUtils.e(TAG, "Error !!! 技能找不到攻击的阵营,放弃此次攻击");
            return;
        }
        ...
        if (effectRange == SKILL_RANGE_SELF) {
        ...
        } else if(SKILL_RANGE_AOE == effectRange){
            // AOE全图技能
            ...
            switch (skillTarget) {
                ...
                case SKILL_TARGET_DISTANCE_NEAR:
                    ...
                    //LogUtils.d(TAG,"farDistance = "+farDistance+" nearDistance="+nearDistance);
        ...
        if (personsBeAttackList == null) {
            LogUtils.e(TAG, "startNormalAttack() 获取不到攻击目标：personsBeAttackList == null");
            return;
        }
    ...
    private void attackSinglePerson(BattlePerson actionPerson, BattlePerson beAttackPerson, SkillBattle skill) {
        ...
        LogUtils.d(TAG, actionPerson.getName() + "的 " + skill.getName() + " 的攻击次数为：" + skill.getAttackTime());
        for (int i = 0; i < skill.getAttackTime(); i++) {
            ...
            if (!RandomUtils.isHappen(accuracy)) {
                BattleLog.log(TAG_MS,actionPerson.getName() + " " + skill.getName() + "(命中率" + accuracy + ") ---> " + beAttackPerson.getName() + " 结果：躲闪");
                continue;
            }
            ...
            BattleLog.log(TAG_CR,actionPerson.getName() + " " + skill.getName() + " ---> " + beAttackPerson.getName() + "(命中率" + accuracy + " 命中了)" + criteString2);
            ...
            switch (skill.getDamageType()) {
                case SKILL_DAMAGE_TYPE_PHYSICS:
                    ...
                    BattleLog.log(TAG_A,skill.getName() + " 物理伤害为" + damage + criteString + " " + beAttackPerson.getName() + " 护甲：" + beAttackPerson.getArmor() + "最终伤害:" + reduceHP);
                    break;
                case SKILL_DAMAGE_TYPE_MAGIC:
                    ...
                    BattleLog.log(TAG_A,skill.getName() + " 魔法伤害为" + damage + criteString + " " + beAttackPerson.getName() + " 魔抗：" + beAttackPerson.getMagicResist() + "最终伤害:" + reduceHP);
                    break;
                case SKILL_DAMAGE_TYPE_REAL:
                    ...
                    BattleLog.log(TAG_A,skill.getName() + " 真实伤害为" + damage + criteString + " " + beAttackPerson.getName() + "最终伤害:" + reduceHP);
                    break;
                case SKILL_DAMAGE_TYPE_PHYSICS_PERCENT:
                    ...
                    BattleLog.log(TAG_A,skill.getName() + " 当前生命" + (int) (skill.getDamageConstant() * 100) + "%物理伤害 " + beAttackPerson.getName() + "当前HP：" + beAttackPerson.getHP_Current() + " 护甲:" + beAttackPerson.getArmor() + " 最终伤害:" + reduceHP);
                    break;
                case SKILL_DAMAGE_TYPE_MAGIC_PERCENT:
                    ...
                    BattleLog.log(TAG_A,skill.getName() + " 当前生命" + (int) (skill.getDamageConstant() * 100) + "%魔法伤害 " + beAttackPerson.getName() + "当前HP：" + beAttackPerson.getHP_Current() + " 魔抗:" + beAttackPerson.getMagicResist() + " 最终伤害:" + reduceHP);
                    break;
                case SKILL_DAMAGE_TYPE_REAL_PERCENT:
                    ...
                    BattleLog.log(TAG_A,skill.getName() + " 当前生命" + (int) (skill.getDamageConstant() * 100) + "%真实伤害 " + beAttackPerson.getName() + "当前HP：" + beAttackPerson.getHP_Current() + " 最终伤害:" + reduceHP);
                    break;
                case SKILL_DAMAGE_TYPE_PHYSICS_PERCENT_MAX:
                    ...
                    BattleLog.log(TAG_A,skill.getName() + " 最大生命百分" + (int) (skill.getDamageConstant() * 100) + "%物理伤害 " + beAttackPerson.getName() + "最大HP：" + beAttackPerson.getHP_MAX() + " 护甲:" + beAttackPerson.getArmor() + " 最终伤害:" + reduceHP);
                    break;
                case SKILL_DAMAGE_TYPE_MAGIC_PERCENT_MAX:
                    ...
                    BattleLog.log(TAG_A,skill.getName() + " 最大生命百分" + (int) (skill.getDamageConstant() * 100) + "%魔法伤害 " + beAttackPerson.getName() + "最大HP：" + beAttackPerson.getHP_MAX() + " 魔抗:" + beAttackPerson.getMagicResist() + " 最终伤害:" + reduceHP);
                    break;
                case SKILL_DAMAGE_TYPE_REAL_PERCENT_MAX:
                    ...
                    BattleLog.log(TAG_A,skill.getName() + " 最大生命百分" + (int) (skill.getDamageConstant() * 100) + "%真实伤害 " + beAttackPerson.getName() + "最大HP：" + beAttackPerson.getHP_MAX() + "最终伤害:" + reduceHP);
                    break;
                default:
                    reduceHP = 0;
                    break;
            ...
            // 触发攻击特效
            if (isTriggerEffect) {
                BattleLog.log(TAG_SE,"触发攻击特效");
                triggerSkillEffect(skill, actionPerson, beAttackPerson);
            } else {
                BattleLog.log(TAG_SE,"没有触发攻击特效");
            }
            ...
            if (remainHp <= 0) {
                remainHp = 0;
                BattleLog.log(TAG_IJ,beAttackPerson.getName() + "受到了" + reduceHP + "点伤害，死亡了");
                beAttackPerson.personDie();
            } else {
                BattleLog.log(TAG_IJ,beAttackPerson.getName() + "受到了" + reduceHP + "点伤害，剩下" + remainHp + "生命值");
            }
    ...
    private void triggerSkillEffect(SkillBattle skill, BattlePerson actionPerson, BattlePerson beAttackPerson) {
        BuffBase buffbase = BuffDataManager.getInstance().getBuffById(skill.getAssistEffect());
        if (buffbase == null) {
            BattleLog.log(TAG_SE,"triggerSkillEffect fail!!! buffbase == null. buffId:"+skill.getAssistEffect());
            return;
        }
        BattleLog.log(TAG_SE,"BuffBase "+buffbase.getName()+" time:"+buffbase.getTime()+" level:"+skill.getLevel());
        BuffEffectBattle buff = new BuffEffectBattle(buffbase, skill.getLevel());
        BattleLog.log(TAG_SE,"技能["+skill.getName()+"]触发了攻击特效["+buff.getName()+"]");
        BattleLog.log(TAG_SE," BuffEffectBattle "+buff.getName()+" = "+buff.getTime());
    ...
    @Override
    public void personDoAction(BattlePerson person) {
